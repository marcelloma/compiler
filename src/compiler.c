...
gcc_jit_rvalue *comp_bin_op_ast(const char *token, json_object *lhs_ast, json_object *rhs_ast, compiler_env *c_env)
{
  ...
  if (strcmp(token, "+") == 0)
  {
    return gcc_jit_context_new_binary_op(c_env->ctx, NULL, GCC_JIT_BINARY_OP_PLUS, c_env->double_type, lhs_rvalue, rhs_rvalue);
  }
  else if (strcmp(token, "-") == 0)
  {
    return gcc_jit_context_new_binary_op(c_env->ctx, NULL, GCC_JIT_BINARY_OP_MINUS, c_env->double_type, lhs_rvalue, rhs_rvalue);
  }
  else if (strcmp(token, "*") == 0)
  {
    return gcc_jit_context_new_binary_op(c_env->ctx, NULL, GCC_JIT_BINARY_OP_MULT, c_env->double_type, lhs_rvalue, rhs_rvalue);
  }
  else if (strcmp(token, "/") == 0)
  {
    return gcc_jit_context_new_binary_op(c_env->ctx, NULL, GCC_JIT_BINARY_OP_DIVIDE, c_env->double_type, lhs_rvalue, rhs_rvalue);
  }
  else
  {
...
void comp_calculated_field(json_object *j_calculated_field, compiler_env *c_env)
{
  ...
  // q: is there a better way to generate this name?
  char *tmp_name[50];
  ...
  gcc_jit_function *func =
      gcc_jit_context_new_function(
          c_env->ctx, NULL, GCC_JIT_FUNCTION_EXPORTED, c_env->double_type, name, 0, NULL, 0);
